...
class SummaryAgent(BaseAgent):
    ...
    def _filter_bullet_points(self, bullets: List[str]) -> List[str]:
        ...
        if filtered_out_count > 0:
            logger.info(f"Filtered out {filtered_out_count} noise/history items")
        
        # Limit to 50 most important bullets to avoid overwhelming LLM
        if len(filtered) > 50:
            original_count = len(filtered)
            logger.info(f"Limiting {original_count} bullets to top 50 (LLM context limit)")
            ...
            logger.info(f"  ├─ {len(priority_filtered[:40])} code files (prioritized)")
            logger.info(f"  └─ {len(non_priority[:10])} docs/tests/config files")
