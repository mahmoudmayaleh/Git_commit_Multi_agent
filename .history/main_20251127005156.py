...
def main():
    ...
    # Always show bullet points
    if state.bullet_points:
        print(f"{Fore.MAGENTA}{Style.BRIGHT}📊 DETECTED CHANGES ({len(state.bullet_points)} items):{Style.RESET_ALL}\n")
        
        # Show first 20 bullet points
        display_count = min(20, len(state.bullet_points))
        for bullet in state.bullet_points[:display_count]:
            # Remove leading bullet if already present
            bullet_text = bullet.lstrip('• ')
            print(f"  {Fore.YELLOW}•{Style.RESET_ALL} {bullet_text}")
        
        if len(state.bullet_points) > display_count:
            remaining = len(state.bullet_points) - display_count
            print(f"\n  {Fore.CYAN}... and {remaining} more changes{Style.RESET_ALL}")
