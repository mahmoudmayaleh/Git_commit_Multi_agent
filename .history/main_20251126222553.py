"""
Git Commit Writer Pipeline - Main CLI Application
...
2. Configure your OpenChat-3.5 settings in .env
3. Stage your changes: git add .
4. Run: python main.py

USAGE:
    python main.py                          # Interactive mode
    python main.py --dry-run                # Generate message without committing
    python main.py --verbose                # Show detailed output
    python main.py --auto-commit            # Commit without confirmation
    python main.py --output commit.txt      # Save to file
...
def main():
    ...
    if args.verbose or args.debug:
        if state.bullet_points:
            print_stage_output("📊 CHANGE ANALYSIS", "", Fore.MAGENTA)
            print_bullet_points(state.bullet_points)
        
        if state.summary:
            print_stage_output("📝 SUMMARY", state.summary, Fore.BLUE)
