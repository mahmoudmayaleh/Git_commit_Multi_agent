...
class CommitPipeline:
    ...
    def run(self, state: Optional[PipelineState] = None) -> PipelineState:
        ...
        try:
            ...
            print(f"  {logger.name if hasattr(logger, 'name') else ''}⏳ Analyzing git diff...")
            state = self._run_agent(self.diff_agent, state, "DIFF ANALYSIS")
            print(f"  ✓ Found {len(state.bullet_points) if state.bullet_points else 0} changes\n")
            
            if state.has_errors():
                return state
            
            # Step 2: SummaryAgent - Create summary
            print(f"  ⏳ Generating summary with AI...")
